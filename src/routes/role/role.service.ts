...
export class RoleService {
  ...
  /**
   * check if the role is one of the 3 base roles
   */
  ...
  async update({ id, data, updatedById }: { id: number; data: UpdateRoleBodyType; updatedById: number }) {
    try {
      ...
      await this.cacheManager.del(`role:${updatedRole.id}`) // delete cache of updated role
      return updatedRole
    } catch (error) {
  ...
  async delete({ id, deletedById }: { id: number; deletedById: number }) {
    try {
      ...
      await this.cacheManager.del(`role:${id}`) // delete cache of deleted role
      return {

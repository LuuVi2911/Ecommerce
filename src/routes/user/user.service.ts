...
export class UserService {
  ...
  /**
   * this function checks if the agent has permission to affect other users.
   * only admin can create admin user, update roleId to admin, delete admin user.
   * if the agent is not admin, then it cannot affect admin
   */
  private async verifyRole({ roleNameAgent, roleIdTarget }) {
    // if the agent is admin, then allow
    if (roleNameAgent === RoleName.Admin) {
      return true
    } else {
      // if the agent is not admin, then roleIdTarget must not be admin
